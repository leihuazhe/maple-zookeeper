...
/**
 * watch机制,首先如果要触发watch事件，需要提前给path，watch一下，watch之后，接下来该节点发生什么事情都会被watch到，
 * 但是watch机制是一次性的，接下来该path如果发生什么CRUD，都不会再触发watch事件，除非在改变该节点之前，重新watch一下。
 * 例子：
 * 创建节点的时候，在createPath方法里，有提前watch该PARENT_PATH，因此创建节点，会有事件触发
 *  if (zkWatch.createPath(PARENT_PATH, System.currentTimeMillis() + "", true)) {
			
			Thread.sleep(1000);
			
		    watch是一次性的，如果读取数据这里这步注释了，就没有watch接下来的更新数据，因此更新数据就不会触发事件
			|
			|		// 读取数据
...
public class SimpleWatcher implements Watcher {
	...
	/** zookeeper服务器地址 */
	private static final String CONNECTION_ADDR = "115.159.41.97";
	/** zk父路径设置 */
...
//		连接成功进来，肯定会触发
	...
	public static void main(String[] args) throws Exception {
		...
		SimpleWatcher zkWatch = new SimpleWatcher();
		//创建连接 
